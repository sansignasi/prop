...
public class Problema {
    ...
    private int MiniMax(Taulell t, int jg, int profunditat) {
        ...
        while (i < p.size()) {
            ...
            if (t.tePiece((int) po.getFirst(), (int) po.getSecond()) && t.getPiece((int) po.getFirst(), (int) po.getSecond()) instanceof King && t.getPiece((int) po.getFirst(), (int) po.getSecond()).getColor() == 'w')
                aux.setRei(false, 0);
            else if (t.tePiece((int) po.getFirst(), (int) po.getSecond()) && t.getPiece((int) po.getFirst(), (int) po.getSecond()) instanceof King && t.getPiece((int) po.getFirst(), (int) po.getSecond()).getColor() == 'b')
                aux.setRei(false, 1);
    ...
    private int valorMax(Taulell t, int jg, int prf) {
        ...
        if (estatTerminal(t, jg, prf)) {
            if (!t.teRei(Math.abs(jg - 1))) return prf;
            else return -1;
        } else {
            vmax = -99999999;
            ArrayList<Pair> p = calculaMovimentsPosibles(t, jg); //no retorna un enter, retorna un conjunt de moviments
